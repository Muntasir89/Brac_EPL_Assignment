import 'package:flutter/material.dart';

class DimenSizes {
  DimenSizes._();
  static const BorderRadius radius_4 = BorderRadius.all(Radius.circular(4));
  static const BorderRadius radius_5 = BorderRadius.all(Radius.circular(5));
  static const BorderRadius radius_8 = BorderRadius.all(Radius.circular(8));
  static const BorderRadius radius_10 = BorderRadius.all(Radius.circular(10));
  static const BorderRadius radius_12 = BorderRadius.all(Radius.circular(12));
  static const BorderRadius radius_16 = BorderRadius.all(Radius.circular(16));
  static const BorderRadius radius_50 = BorderRadius.all(Radius.circular(50));
}

class Gap {
  /// Constant gap widths
  static const shrinkGap = SizedBox.shrink();
  static const w2 = SizedBox(width: 2);
  static const w4 = SizedBox(width: 4);
  static const w8 = SizedBox(width: 8);
  static const w10 = SizedBox(width: 10);
  static const w12 = SizedBox(width: 12);
  static const w14 = SizedBox(width: 14);
  static const w16 = SizedBox(width: 16);
  static const w20 = SizedBox(width: 20);
  static const w24 = SizedBox(width: 24);
  static const w28 = SizedBox(width: 28);
  static const w32 = SizedBox(width: 32);
  static const w36 = SizedBox(width: 36);
  static const w40 = SizedBox(width: 40);
  static const w48 = SizedBox(width: 48);
  static const w56 = SizedBox(width: 56);
  static const w64 = SizedBox(width: 64);
  static const w72 = SizedBox(width: 72);
  static const w80 = SizedBox(width: 80);
  static const w150 = SizedBox(width: 150);

  /// Constant gap heights
  static const h2 = SizedBox(height: 2);
  static const h4 = SizedBox(height: 4);
  static const h5 = SizedBox(height: 5);
  static const h8 = SizedBox(height: 8);
  static const h10 = SizedBox(height: 10);
  static const h11 = SizedBox(height: 11);
  static const h12 = SizedBox(height: 12);
  static const h14 = SizedBox(height: 14);
  static const h16 = SizedBox(height: 16);
  static const h18 = SizedBox(height: 18);
  static const h20 = SizedBox(height: 20);
  static const h22 = SizedBox(height: 22);
  static const h24 = SizedBox(height: 24);
  static const h25 = SizedBox(height: 25);
  static const h26 = SizedBox(height: 26);
  static const h28 = SizedBox(height: 28);
  static const h32 = SizedBox(height: 32);
  static const h34 = SizedBox(height: 34);
  static const h36 = SizedBox(height: 36);
  static const h37 = SizedBox(height: 37);
  static const h40 = SizedBox(height: 40);
  static const h43 = SizedBox(height: 43);
  static const h46 = SizedBox(height: 46);
  static const h48 = SizedBox(height: 48);
  static const h50 = SizedBox(height: 50);
  static const h56 = SizedBox(height: 56);
  static const h57 = SizedBox(height: 57);
  static const h60 = SizedBox(height: 60);
  static const h64 = SizedBox(height: 64);
  static const h72 = SizedBox(height: 72);
  static const h74 = SizedBox(height: 74);
  static const h80 = SizedBox(height: 80);
  static const h100 = SizedBox(height: 100);
  static const h120 = SizedBox(height: 120);
  static const h182 = SizedBox(height: 182);
}
